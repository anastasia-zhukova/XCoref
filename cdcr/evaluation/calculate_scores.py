...
from cdcr.config import LOGGER, RUN_PERL_SCORER, EXCLUDE_ACT_G
from cdcr import config
...
WCL, NUMBER, NUMBER_ENT, UNIQUE_LEMMAS = "wcl", "mentions_num", "entities_num", "unique_lemmas"
TIME = "time"
AVG_TIME = "avg_time"
...
class EvalScorer:
    ...
    def evaluate_entities(self):
        ...
        merge_step_df = pd.DataFrame()
        merge_step_type_df = pd.DataFrame()
        ...
        for group_id, (group_name_tuple, group_df) in enumerate(param_df_groups):
            ...
            if EXCLUDE_ACT_G:
                pred_true_df_filt = pred_true_df[pred_true_df[ANNOT_TYPE] != "ACTOR-G"]
            else:
                pred_true_df_filt = pred_true_df
            ...
            if update_dataset_stats:
                dataset_stats_df, entities_stats_df = self.calc_wcl(self.datasets[group_name_tuple[0]],
                                                                    group_name_tuple[0])
                dataset_stats_df.index = [
                    shortuuid.uuid(dataset_stats_df.loc[ind, DATASET] + dataset_stats_df.loc[ind, CONCEPT_TYPE])
            ...
            # general metrics
            scores_df = self.calc_entity_metrics(pred_true_df_filt, group_name, group_name_tuple[0])
            ...
            # per step metrics
            merge_docsets_list = [entities_docsets[set_id] for set_id in list(group_df.index)
                                  if entities_docsets[set_id].entities.evaluation_details is not None]
            if len(merge_docsets_list) and not any([UNK in v for v in list(pred_true_df_filt[TRUE_LABEL].values)]):
                all_exec_details_df = pd.DataFrame()
                for docset in merge_docsets_list:
                    all_exec_details_df = all_exec_details_df.append(docset.entities.evaluation_details)

                step_columns = [col for col in list(all_exec_details_df.columns) if "_entname" in col]
                for step_col in step_columns:
                    step_df = pred_true_df_filt.drop([PRED_LABEL], axis=1)
                    step_df["orig_mention"] = ["_".join(ind.split("_")[:-1]) for ind in list(step_df.index)]
                    step_df = pd.merge(step_df, all_exec_details_df[[step_col]], left_on=["orig_mention"],
                                       right_index=True,
                                       how="left").rename(columns={step_col: PRED_LABEL})
                    step_df[PRED_LABEL] = [
                        "_".join([step_df[PRED_LABEL].values[i], step_df["orig_mention"].values[i].split("_")[0]])
                        for i in range(len(step_df))]
                    step_df.drop_duplicates(inplace=True)
                    scores_steps_df = self.calc_entity_metrics(step_df, group_name, group_name_tuple[0])
                    scores_steps_df["step"] = [step_col.replace("_entname", "")] * len(scores_steps_df)
                    scores_steps_df["config"] = [group_name] * len(scores_steps_df)
                    merge_step_df = merge_step_df.append(scores_steps_df)

                    # entity type metrics
                    types_step_df = self.calc_entity_type_metrics(step_df, group_name, group_name_tuple[0])
                    types_step_df["step"] = [step_col.replace("_entname", "")] * len(types_step_df)
                    types_step_df["config"] = [group_name] * len(types_step_df)
                    merge_step_type_df = merge_step_type_df.append(types_step_df)

            # entity type metrics
            types_df = self.calc_entity_type_metrics(pred_true_df_filt, group_name, group_name_tuple[0])
        ...
        bar.finish()
        return [eval_results_df, eval_results_type_df, all_datasets_df, performance_df, merge_step_df,
                merge_step_type_df]
    ...
    def calc_entity_metrics(self, pred_true_df, index, dataset_name):
        ...
        if RUN_PERL_SCORER:
            ...
            with open(os.path.join(config.TMP_PATH, "key.key_conll"), "w") as file:
                file.write(in_str)
            with open(os.path.join(config.TMP_PATH, "response.response_conll"), "w") as file:
                file.write(out_str)
            result_file = os.path.join(config.TMP_PATH, "conll_res.txt")

            scorer_command = (f'perl {os.path.join(config.ROOT_DIR,"cdcr/entities/eecdcr/scorer/scorer.pl")} '
                              f'all {os.path.join(config.TMP_PATH, "key.key_conll")} '
            ...
            for metrics_name, metrics_vals in f1_dict.items():
                ...
                metrics_df = pd.concat([metrics_df, pd.DataFrame({
                    R + metrics_name: metrics_vals[R],
                    P + metrics_name: metrics_vals[P],
                    F1 + metrics_name: metrics_vals[F1]
    ...
    def _muc(self, true_dict, pred_dict, dataset_name=None):
        ...
        for true_label, true_cands in true_dict.items():
            ...
            if len(true_cands) != len(partitions_union):
                for v in set(true_cands).difference(partitions_union):
                    partitions.append([v])

            sum_nom_r += len(true_cands) - len(partitions)
            sum_denom_r += len(true_cands) - 1
        recall = sum_nom_r / sum_denom_r if sum_denom_r > 0 else 0
        ...
        for pred_label, pred_cands in pred_dict.items():
            ...
            sum_nom_p += len(pred_cands) - len(partitions)
            sum_denom_p += len(pred_cands) - 1

        precision = sum_nom_p / sum_denom_p if sum_denom_p > 0 else 0
