...
import numpy as np
import progressbar
from cdcr.entities.eecdcr.src.all_models.scorer import *
from cdcr.entities.eecdcr.src.shared.eval_utils import *
import _pickle as cPickle
from cdcr.entities.eecdcr.src.all_models.bcubed_scorer import *
...
def load_predicted_topics(test_set, config):
    '''
    ...
    :param test_set: A Corpus object represents the test set
    :param config: a configuration dictionary that contains a path to a file
    ...
    '''
    new_topics = {}
    with open(config.predicted_topics_path, 'rb', encoding="utf-8") as f:
...
def load_entity_wd_clusters(config):
    '''
    ...
    with open(config.wd_entity_coref_file, 'r', encoding="utf-8") as js_file:
...
def load_embeddings(embed_path, vocab_path):
    ...
    vocabulary.
    '''
    with open(embed_path,'rb', encoding="utf-8") as f:
...
def write_event_coref_results(corpus, out_dir, config):
    '''
    ...
    '''
    if not config.test_use_gold_mentions:
        ...
        write_span_based_cd_coref_clusters(corpus, out_file, is_event=True, is_gold=False,
                                           use_gold_mentions=config.test_use_gold_mentions)
    else:
...
def write_entity_coref_results(corpus, out_dir,config):
    '''
...
def generate_cluster_pairs(clusters, is_train):
    ...
    widgets = [
        progressbar.FormatLabel("PROGRESS: Formed %(value)d/%(max_value)d (%(percentage)d %%) cluster pairs (in: %(elapsed)s).")
    ]
    bar = progressbar.ProgressBar(widgets=widgets,
                                  maxval=len(clusters) * len(clusters) / 2).start()
    c_num = 0
...
def merge(clusters, cluster_pairs, other_clusters,model, device, topic_docs, epoch, topics_counter,
    ...
    widgets = [
        progressbar.FormatLabel("PROGRESS: Calculated score for %(value)d/%(max_value)d (%(percentage)d %%) cluster pairs "
                                "(in: %(elapsed)s).")
    ]
    bar = progressbar.ProgressBar(widgets=widgets,
                                  maxval=len(cluster_pairs)).start()

    for cl_id, pair in enumerate(cluster_pairs):
        pair_score = assign_score(pair, model, device, topic_docs, is_event,
                                  use_args_feats,use_binary_feats, other_clusters)
    ...
    while True:
        ...
        if max_score > threshold:
            ...
            #     str(max_pair[1])))
            logging.info('epoch {} topic {}/{} - merge {} clusters with score {} \nclusters : {} {}'.format(
                epoch, topics_counter, topics_num, mode, str(max_score), str(max_pair[0]),
                str(max_pair[1])))
...
def test_models(test_set, cd_event_model, cd_entity_model, device,
                config, write_clusters, out_dir, doc_to_entity_mentions, analyze_scores):
    '''
    ...
    :param device: Pytorch device
    :param config: a dictionary contains the experiment's configurations
    ...
    if config.load_predicted_topics:
        topics = load_predicted_topics(test_set, config) # use the predicted sub-topics
    else:
    ...
    with torch.no_grad():
        for topic_id in topics_keys:
            ...
            event_mentions, entity_mentions = topic_to_mention_list(topic, is_gold=config.test_use_gold_mentions)
            ...
            entity_th = config.entity_merge_threshold
            event_th = config.event_merge_threshold

            for i in range(1, config.merge_iters + 1):
                ...
                test_model(clusters=topic_entity_clusters, other_clusters=topic_event_clusters,
                           ...
                           threshold=entity_th,
                           use_args_feats=config.use_args_feats,
                           use_binary_feats=config.use_binary_feats)
                # Merge events
            ...
            set_coref_chain_to_mentions(topic_event_clusters, is_event=True,
                                        is_gold=config.test_use_gold_mentions, intersect_with_gold=True)
            ...
            if write_clusters:
                ...
                with open(os.path.join(out_dir, 'entity_clusters.txt'), 'a', encoding="utf-8") as entity_file_obj:
                ...
                with open(os.path.join(out_dir, 'event_clusters.txt'), 'a', encoding="utf-8") as event_file_obj:
        ...
        if write_clusters:
            write_event_coref_results(test_set, out_dir, config)
            write_entity_coref_results(test_set, out_dir, config)
    ...
    if analyze_scores:
        ...
        with open(os.path.join(out_dir,'test_topics'), 'wb', encoding="utf-8") as f:
            cPickle.dump(topics, f)

    ev_f1, en_f1 = 0,0
    # if config.test_use_gold_mentions:
    ...
    #
    #     ev_f1, en_f1 = event_b3_f1, entity_b3_f1
    #     logging.info('Event F1 score: ' + str(ev_f1))
    #     logging.info('Entity F1 score: ' + str(en_f1))
    #
    # else:
    #     logging.info('Using predicted mentions, can not calculate CoNLL F1')

    return all_entity_clusters, all_event_clusters, en_f1, ev_f1
...
def save_mention_representations(clusters, out_dir, is_event):
    ...
    with open(os.path.join(out_dir, filename), 'wb', encoding="utf-8") as f:
