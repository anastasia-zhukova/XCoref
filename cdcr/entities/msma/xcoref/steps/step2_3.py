...
class XCorefStep3(MergeStep):
    ...
    def __init__(self, step, docs, entity_dict, ent_preprocessor, model):
        ...
        super().__init__(MSMA2_3, step, docs.configuration.entity_identifier_config, entity_dict, ent_preprocessor,
                         model)
    ...
    def merge(self) -> dict:
        ...
        for key, entity in group_ne_entities.items():
            group_ne_dict[key] = set([k.lower() for k,v in entity.headwords_cand_tree.items() if len(v) > 1])
        ...
        for entity_key, gr_entity in group_entities.items():
            for entity_ne_key, ne_heads in group_ne_dict.items():
                if len(set(gr_entity.headwords_cand_tree).intersection(ne_heads)):
                    try:
                        gr_entity.absorb_entity(self.entity_dict[entity_ne_key], self.step_name + MSMA2_3_SUB_1, 1.0)
    ...
    #     if ent.type in [PERSON_NES_TYPE, PERSON_NN_TYPE, PERSON_NNS_TYPE, GROUP_TYPE]:
    #         to_save[ent.representative] = {"hist": ent.merge_history, "all": [m.text for m in ent.members]}
    # with open("37.json", "w") as file:
    ...
    def preprocess_entities(self):
        ...
        for key, entity in self.non_ne_entities.items():
            ...
            if len(set().union(*phrases)) == len(headwords) and entity.type != GROUP_TYPE:
                adapted_words = self.model.optimize_phrase(MergeStep.words_to_tokens(set().union(*headwords).union(*nmod_phrases),
                                                                          token_dict), True)
                adapted_words = self.model.optimize_phrase(MergeStep.words_to_tokens(adapted_words, token_dict), True)
                ...
                adapted_words = self.model.optimize_phrase(MergeStep.words_to_tokens(set().union(*phrases), token_dict), True)

            if self.params.max_repr_words >= len(adapted_words) > 0:
                phrase = [adapted_words]
            else:
                phrase = [self.model.optimize_phrase(MergeStep.words_to_tokens([list(entity.headwords_cand_tree.keys())[0]],
                                                                    token_dict), True)]
            ...
            head_words = self.model.optimize_phrase(MergeStep.words_to_tokens(list(entity.headwords_cand_tree.keys()), token_dict)
                                         , True)
            if len(head_words) == 0:
                continue
            ...
            head_set = head_set.union(
                self.model.optimize_phrase(MergeStep.words_to_tokens(list(entity.headwords_cand_tree.keys()), token_dict), True))
    ...
    def create_entities(self, core_clusters_no_aliens, clusters_body_no_aliens, clusters_final, points_to_add,
        ...
        for ph in set().union(*clusters_final):
            ...
            if len(self.phrases_non_ne_entities[key_set]) > 1 or key_set in self.big_to_small_dict:
                ...
                for key in self.phrases_non_ne_entities[key_set][1:]:
                    entity.absorb_entity(self.entity_dict[key], self.step_name + MSMA2_3_SUB_0, 1.0)
                    appendix = MSMA2_3_SUB_0
                ...
                if key_set in self.big_to_small_dict:
                    ...
                    for key in dep_entity_keys:
                        entity.absorb_entity(self.entity_dict[key], self.step_name + MSMA2_3_SUB_1, 1.0)
                        appendix = MSMA2_3_SUB_1
        ...
        for phrase_set in clusters_final:
            ...
            prir_str_dict = {0: "", 1: MSMA2_3_SUB_2, 2: MSMA2_3_SUB_3, 3: MSMA2_3_SUB_4, 4: MSMA2_3_SUB_5}
