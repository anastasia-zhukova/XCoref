...
class XCorefStep2(MergeStep):
    """
    A step merges entities using similarity of core mentions to non-NE mentions in cores.
    """

    def __init__(self, step, docs, entity_dict, ent_preprocessor, model):

        super().__init__(MSMA2_2, step, docs.configuration.entity_identifier_config, entity_dict, ent_preprocessor,
                         model)
    ...
    def _build_vectors(self, entity_dict):
        ...
        for key, ent in entity_dict.items():
            ...
            for phrase in phrase_set:
                local_vector_df = local_vector_df.append(pd.DataFrame(self.model.query(phrase.split(" ")), columns=columns,
                                                          index=[phrase]))
