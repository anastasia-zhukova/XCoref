...
class SieveBasedApproach(Identifier):
    """
    A multistep merging approach (SIEVE_BASED) identifies searches for similar entities by appliying different similarity
    criteria at each merge step and merge entities together if they are identified as similar.

    SIEVE_BASED exists in two versions, where we experimented with different variations of merge steps and ways how to
    process entities.
    """

    logger = LOGGER

    def __init__(self, docs, entity_class=EntitySieves, ent_preprocessor=EntityPreprocessorSieves):
        ...
        # steps and entity need to be defined for a specific SIEVE_BASED implementation
        self.steps = {}
    ...
    def extract_entities(self):
        ...
        if INIT_RUN:
            if self.docs.configuration.entity_identifier_config.load_preproc:
                if any([file for file in os.listdir(TMP_PATH) if self.docs.topic.split("_")[0] in file and XCOREF in file]):
    ...
    def load_interm_results(self):
        ...
        """
        topic_id = self.docs.topic.split("_")[0]
        approach = self.docs.configuration.entity_method if self.docs.configuration.entity_method != XCOREF_BASE else XCOREF
