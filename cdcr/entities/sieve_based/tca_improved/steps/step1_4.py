...
class TCAStep4(Sieve):
    ...
    def __init__(self, step, docs, entity_dict, ent_preprocessor, model):
        ...
        super().__init__(TCA_4, step, docs.configuration.entity_identifier_config, entity_dict, ent_preprocessor,
                         model)

        for ent in list(self.entity_dict.values()):
            # freq_phrases = [self.remove_oov(MergeStep.words_to_tokens(list(phrase), ent.token_dict),
            freq_phrases = [self.model.optimize_phrase(Sieve.words_to_tokens(list(phrase), ent.token_dict),
