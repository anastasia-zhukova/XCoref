...
class TCAStep2(Sieve):
    ...
    def __init__(self, step, docs, entity_dict, ent_preprocessor, model):
        ...
        super().__init__(TCA_2, step, docs.configuration.entity_identifier_config, entity_dict, ent_preprocessor,
                         model)
        ...
        for ent in list(self.entity_dict.values()):
            wv_labeling = list(filter(lambda x: len(x),
                                      ...
                                      [self.model.optimize_phrase(Sieve.words_to_tokens(list(phrase), ent.token_dict), True)
