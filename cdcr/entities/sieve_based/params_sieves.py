...
class SievesParams(ParamsEntities):
    """
    A general class of parameters for multi-step-merging approach.
    Table values and parameters are different in each SIEVE_BASED implementation.
    """
    ...
    def get_default_params(self, def_folder_path: str):
        ...
        self.tables = SievesParams.read_comp_tables(os.path.join(config.NEWALYZE_PATH, def_folder_path, COMP_TABLES_NAME))
        return self
    ...
    def read_config(self, config_path):
        ...
        user_tables = SievesParams.read_comp_tables(os.path.join(config_path, COMP_TABLES_NAME))
